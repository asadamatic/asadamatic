...
class TodoHomeController extends GetxController {
  ...
  List<TaskData> getList() {
    return tasks.where((taskData) => taskData.date == DateFormat('d/M/y').format(pickedDate)).toList();
  }
  ...
  updateDataForGraph() async {
    ...
    for (int day = 1;
      ...
      int done = getListNew('$day/$monthYear').where((taskData) => taskData.status == true).length;
